...
for ind in data.index:
    ...
    if (data["ProjectId"][ind]+'.txt') not in os.listdir('./raw-data/fine-scale/UK/UKRI/titles-abstracts/'):
        print(ind)
        #request data
        ...
        if r.status_code == 200:
            json_r = r.json()
            with open('./raw-data/fine-scale/UK/UKRI/titles-abstracts/'+ data["ProjectId"][ind]+'.txt', 'w') as f:
